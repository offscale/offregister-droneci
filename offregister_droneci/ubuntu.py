...
def install0(*args, **kwargs):
    ...
    if "DRONE_SERVER_PORTS" in kwargs:
        compose["services"]["drone-server"]["ports"] = kwargs["DSP"] = kwargs.pop(
            "DRONE_SERVER_PORTS"
        )
    ...
    if "DRONE_GITHUB" in kwargs and kwargs["DRONE_GITHUB"]:
        compose["services"]["drone-server"]["environment"] = [
            "{}={}".format(k, _json_bool(kwargs[k]))
            for k in kwargs
            if k.startswith("DRONE_")
        ]
        ...
        if "ports" in compose["services"]["drone-server"]:
            compose["services"]["drone-agent"]["environment"].append(
                "DRONE_SERVER=localhost:{}".format(
                    compose["services"]["drone-server"]["ports"][1]
                )
            )
...
def configure_nginx1(*args, **kwargs):
    ...
    dumps(
        utils.upsert_by_location(
            "/drone",
            conf,
            emit.api_proxy_block(
                "/drone",
                "https://localhost:{}".format(
                    kwargs.get("DSP", kwargs.get("DRONE_SERVER_PORTS", (None, None)))[1]
                    or "9000"
                ),
            ),
        )
